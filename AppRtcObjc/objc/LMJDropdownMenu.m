...
- (void)initPropertys{
    ...
    _titleBgColor        = [UIColor colorWithRed:255/255.f green:255/255.f blue:255/255.f alpha:1];
    _titleFont           = [UIFont systemFontOfSize:13];
    _titleColor          = [UIColor blackColor];
    _titleAlignment      = NSTextAlignmentLeft;
    _titleEdgeInsets     = UIEdgeInsetsMake(3, 8, 0, 0);
    
    _rotateIcon          = nil;
    _rotateIconSize      = CGSizeMake(15, 15);
    
    _optionBgColor       = [UIColor colorWithRed:255/255.f green:255/255.f blue:255/255.f alpha:1];
...
- (void)initViews{
    ...
    _mainBtn.titleEdgeInsets            = UIEdgeInsetsMake(5, 0, 0, 0);
...
- (void)setFrame:(CGRect)frame{
    ...
    [_mainBtn.topAnchor constraintEqualToAnchor:self.topAnchor].active = YES;
    [_mainBtn.leftAnchor constraintEqualToAnchor:self.leftAnchor].active = YES;
    [_mainBtn.rightAnchor constraintEqualToAnchor:self.rightAnchor].active = YES;
    [_mainBtn.heightAnchor constraintEqualToConstant:_originHeight].active = YES;
    
    [_arrowMark.centerYAnchor constraintEqualToAnchor:_mainBtn.centerYAnchor].active = YES;
    [_arrowMark.leftAnchor constraintEqualToAnchor:self.rightAnchor constant:-(7.5 + self.rotateIconSize.width)].active = YES;
...
- (void)selectItme:(int)index{
    if(index >= (int)[self.dataSource numberOfOptionsInDropdownMenu:self])
        return;
    self.title = [self.dataSource dropdownMenu:self titleForOptionAtIndex:index];
}

- (int)getSelcetedIndex{
    NSUInteger count = [self.dataSource numberOfOptionsInDropdownMenu:self];
    for (int i = 0; i < (int)count; i++) {
        if([[self.dataSource dropdownMenu:self titleForOptionAtIndex:i] compare:self.title] == 0){
            return i;
        }
    }
    return 0;
