...
static inline void DrawFunction(HDC hDC, bool bTransparent, UiRect rcDest, HDC hdcSrc, UiRect rcSrc, bool bAlphaChannel, int uFade)
{
	if (bTransparent || bAlphaChannel || uFade < 255
		...
		BLENDFUNCTION ftn = { AC_SRC_OVER, 0, uFade, AC_SRC_ALPHA };
          ::AlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.GetWidth(),
                           rcDest.GetHeight(),
...
bool RenderContext_GdiPlus::AlphaBlendself(int xDest, int yDest, int widthDest, int heightDest, HDC hdcSrc, int xSrc, int yScr, int widthSrc, int heightSrc, BYTE uFade /*= 255*/)
{
...
void RenderContext_GdiPlus::DrawRoundRect(const UiRect& rc, const CSize& roundSize, int nSize, DWORD dwPenColor)
{
	...
	Gdiplus::GraphicsPath pPath;
        ...
        pPath.AddArc((INT)(rc.right - roundSize.cx), rc.top, roundSize.cx,
                     roundSize.cy, 270, 90);
        pPath.AddLine((INT)(rc.right), rc.top + roundSize.cy, rc.right, rc.bottom - roundSize.cy);
        pPath.AddArc((INT)(rc.right - roundSize.cx), rc.bottom - roundSize.cy,
                     roundSize.cx, roundSize.cy, 0, 90);
        pPath.AddLine((INT)(rc.right - roundSize.cx), rc.bottom,
                      rc.left + roundSize.cx, rc.bottom);
        pPath.AddArc((INT)rc.left, rc.bottom - roundSize.cy, roundSize.cx,
                     roundSize.cy, 90, 90);
