...
Window::Window()
  ...
  if (GlobalManager::GetDefaultFontName().length() > 0)
    _tcscpy_s(lf.lfFaceName, LF_FACESIZE,
              GlobalManager::GetDefaultFontName().c_str());
...
bool Window::RegisterSuperClass() {
  ...
  if (!::GetClassInfoEx(NULL, superClassName.c_str(), &wc)) {
    if (!::GetClassInfoEx(::GetModuleHandle(NULL), superClassName.c_str(),
                          &wc)) {
...
void Window::Close(UINT nRet) {
  ...
  if (m_pRoot && IsWindowsVistaOrGreater()) {
    ...
    TimerManager::GetInstance()->AddCancelableTimer(m_closeFlag.GetWeakFlag(),
                                                    closeCallback, 300, 1);
  } else {
...
void Window::SetIcon(UINT nRes) {
  HICON hIcon = (HICON)::LoadImage(
      ::GetModuleHandle(NULL), MAKEINTRESOURCE(nRes), IMAGE_ICON,
      ::GetSystemMetrics(SM_CXICON), ::GetSystemMetrics(SM_CYICON),
      LR_DEFAULTCOLOR | LR_SHARED);
  ASSERT(hIcon);
  ::SendMessage(m_hWnd, WM_SETICON, (WPARAM)TRUE, (LPARAM)hIcon);
  hIcon = (HICON)::LoadImage(::GetModuleHandle(NULL), MAKEINTRESOURCE(nRes),
...
LRESULT Window::SendMessage(UINT uMsg,
                            WPARAM wParam /*= 0*/,
                            LPARAM lParam /*= 0*/) {
  ASSERT(::IsWindow(m_hWnd));
...
LRESULT Window::PostMessage(UINT uMsg,
                            WPARAM wParam /*= 0*/,
                            LPARAM lParam /*= 0*/) {
  ASSERT(::IsWindow(m_hWnd));
...
LRESULT CALLBACK Window::__WndProc(HWND hWnd,
                                   UINT uMsg,
                                   WPARAM wParam,
                                   LPARAM lParam) {
  Window* pThis = NULL;
...
LRESULT CALLBACK Window::__ControlProc(HWND hWnd,
                                       UINT uMsg,
                                       WPARAM wParam,
                                       LPARAM lParam) {
  Window* pThis = NULL;
...
bool Window::AttachDialog(Box* pRoot) {
  ...
  // Remove the existing control-tree. We might have gotten inside this function
  // as a result of an event fired or similar, so we cannot just delete the
  // objects and pull the internal memory of the calling code. We'll delay the
  // cleanup.
  if (m_pRoot != NULL) {
...
bool Window::InitControls(Control* pControl, Box* pParent /*= NULL*/) {
  ...
  if (pControl == NULL)
    return false;
  pControl->SetWindow(this, pParent != NULL ? pParent : pControl->GetParent(),
                      true);
...
void Window::AddClass(const std::wstring& strClassName,
                      const std::wstring& strControlAttrList) {
...
std::wstring Window::GetClassAttributes(
    const std::wstring& strClassName) const {
...
bool Window::AddOptionGroup(const std::wstring& strGroupName,
                            Control* pControl) {
...
std::vector<Control*>* Window::GetOptionGroup(
    const std::wstring& strGroupName) {
  auto it = m_mOptionGroup.find(strGroupName);
  if (it != m_mOptionGroup.end())
    return &(it->second);
  return NULL;
...
void Window::RemoveOptionGroup(const std::wstring& strGroupName,
                               Control* pControl) {
...
void Window::SetTextId(const std::wstring& strTextId) {
  ::SetWindowText(
      m_hWnd, MutiLanSupport::GetInstance()->GetStringViaID(strTextId).c_str());
}
...
void Window::SetPos(const UiRect& rc,
                    bool bNeedDpiScale,
                    UINT uFlags,
  ...
  ::SetWindowPos(m_hWnd, hWndInsertAfter, rcNewPos.left, rcNewPos.top,
                 rcNewPos.GetWidth(), rcNewPos.GetHeight(), uFlags);
}
...
void Window::SetInitSize(int cx,
                         int cy,
                         bool bContainShadow,
                         bool bNeedDpiScale) {
  if (bNeedDpiScale) {
  ...
  if (m_pRoot == NULL && m_hWnd != NULL) {
    ::SetWindowPos(m_hWnd, NULL, 0, 0, m_szInitWindowSize.cx,
                   m_szInitWindowSize.cy,
                   SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
  }
}

bool Window::AddMessageFilter(IUIMessageFilter* pFilter) {
  ASSERT(std::find(m_aMessageFilters.begin(), m_aMessageFilters.end(),
                   pFilter) == m_aMessageFilters.end());
  m_aMessageFilters.push_back(pFilter);
  return true;
}
bool Window::RemoveMessageFilter(IUIMessageFilter* pFilter) {
  for (auto it = m_aMessageFilters.begin(); it != m_aMessageFilters.end();
       it++) {
    if (*it == pFilter) {
...
bool Window::AddControlFromPointFinder(IControlFromPointFinder* pFinder) {
  ASSERT(std::find(m_aIControlFromPointFinder.begin(),
                   m_aIControlFromPointFinder.end(),
                   pFinder) == m_aIControlFromPointFinder.end());
  m_aIControlFromPointFinder.emplace_back(pFinder);
  return true;
}
bool Window::RemoveControlFromPointFinder(IControlFromPointFinder* pFinder) {
  auto it = std::find_if(
      m_aIControlFromPointFinder.begin(), m_aIControlFromPointFinder.end(),
      [&](IControlFromPointFinder* item) { return pFinder == item; });
...
bool Window::AddTranslateAccelerator(
    ITranslateAccelerator* pTranslateAccelerator) {
  ASSERT(std::find(m_aTranslateAccelerator.begin(),
                   m_aTranslateAccelerator.end(),
                   pTranslateAccelerator) == m_aTranslateAccelerator.end());
  m_aTranslateAccelerator.push_back(pTranslateAccelerator);
  return true;
}

bool Window::RemoveTranslateAccelerator(
    ITranslateAccelerator* pTranslateAccelerator) {
...
LRESULT Window::DoHandlMessage(UINT uMsg,
                               WPARAM wParam,
                               LPARAM lParam,
                               bool& handled) {
  handled = false;
  ...
  switch (uMsg) {
    case WM_APP + 1: {
      for (auto it = m_aDelayedCleanup.begin(); it != m_aDelayedCleanup.end();
           it++)
        delete *it;
    ...
    case WM_MOUSEMOVE: {
      ...
      if (m_pEventClick != NULL) {
        m_pEventClick->HandleMessageTemplate(kEventMouseMove, wParam, lParam, 0,
                                             pt);
      } else if (m_pNewHover != NULL) {
        m_pNewHover->HandleMessageTemplate(kEventMouseMove, wParam, lParam, 0,
                                           pt);
      }
    } break;
    case WM_LBUTTONDOWN: {
      ...
      pControl->HandleMessageTemplate(kEventMouseButtonDown, wParam, lParam, 0,
                                      pt);
    } break;
    case WM_RBUTTONDOWN: {
      if (m_pEventTouch != NULL || m_pEventPointer != NULL)
      ...
      pControl->HandleMessageTemplate(kEventMouseRightButtonDown, wParam,
                                      lParam, 0, pt);
    } break;
    case WM_LBUTTONDBLCLK: {
      ...
      pControl->HandleMessageTemplate(kEventInternalDoubleClick, wParam, lParam,
                                      0, pt);
    } break;
    case WM_LBUTTONUP: {
      POINT pt = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
      ...
      m_pEventClick->HandleMessageTemplate(kEventMouseButtonUp, wParam, lParam,
                                           0, pt);
      m_pEventClick = NULL;
    } break;
    case WM_RBUTTONUP: {
      ...
      m_pEventClick->HandleMessageTemplate(kEventMouseRightButtonUp, wParam,
                                           lParam, 0, pt);
      // m_pEventClick = NULL;
    } break;
    case WM_IME_STARTCOMPOSITION: {
      if (m_pFocus == NULL)
        break;
      m_pFocus->HandleMessageTemplate(kEventImeStartComposition, wParam, lParam,
                                      wParam);
    } break;
    case WM_IME_ENDCOMPOSITION: {
      if (m_pFocus == NULL)
        break;
      m_pFocus->HandleMessageTemplate(kEventImeEndComposition, wParam, lParam,
                                      wParam);
    } break;
    case WM_MOUSEWHEEL: {
      POINT pt = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
    ...
    case WM_TOUCH: {
      ...
      // 只关心第一个触摸位置
      if (nNumInputs >= 1 &&
          GetTouchInputInfoWrapper((HTOUCHINPUT)lParam, nNumInputs, pInputs,
                                   sizeof(TOUCHINPUT))) {
        if (pInputs[0].dwID != 0) {
          POINT pt;
          ...
          if (pInputs[0].dwFlags & TOUCHEVENTF_DOWN) {
            ...
            m_pEventTouch->HandleMessageTemplate(kEventTouchUp, 0, lParam, 0,
                                                 pt);
            m_pEventTouch = NULL;
          }
        }
    ...
    case WM_POINTERCAPTURECHANGED: {
      ...
      switch (uMsg) {
        case WM_POINTERDOWN: {
          ...
          pControl->HandleMessageTemplate(kEventPointDown, 0, lParam, 0, pt,
                                          pressure);
        ...
        case WM_POINTERUPDATE:
          ...
          m_pEventPointer->HandleMessageTemplate(kEventPointMove, 0, 0, 0, pt,
                                                 pressure);
        ...
        case WM_POINTERCAPTURECHANGED:
          ...
          m_pEventPointer->HandleMessageTemplate(kEventPointUp, 0, lParam, 0,
                                                 pt, pressure);
    ...
    case WM_KILLFOCUS: {
      ...
      POINT pt = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
      m_ptLastMousePos = pt;
      if (pControl == NULL || !pControl->IsNeedButtonUpWhenKillFocus())
        break;
      ReleaseCapture();

      pControl->HandleMessageTemplate(kEventMouseButtonUp, wParam, lParam, 0,
                                      pt);
      m_pEventClick = NULL;
      m_pEventTouch = NULL;
      m_pEventPointer = NULL;
    ...
    case WM_CONTEXTMENU: {
      ...
      m_pEventClick->HandleMessageTemplate(kEventInternalMenu, wParam,
                                           (LPARAM)m_pEventClick, 0, pt);
      m_pEventClick = NULL;
    } break;
    case WM_CHAR: {
      if (m_pFocus == NULL)
    ...
    case WM_KEYDOWN: {
      ...
      if (wParam == VK_TAB) {
        if (m_pFocus && m_pFocus->IsVisible() && m_pFocus->IsEnabled() &&
            dynamic_cast<RichEdit*>(m_pFocus) != nullptr) {
    ...
    case WM_NOTIFY: {
      ...
      if (lpNMHDR != NULL)
        return ::SendMessage(lpNMHDR->hwndFrom, OCM__BASE + uMsg, wParam,
                             lParam);
      handled = true;
      return 0;
    } break;
...
bool Window::HandleMouseEnterLeave(const POINT& pt,
                                   WPARAM wParam,
                                   LPARAM lParam) {
...
void Window::SetFocus(Control* pControl) {
  ...
  if (hFocusWnd != m_hWnd && pControl != m_pFocus && pControl != NULL)
    ::SetFocus(m_hWnd);
  // Already has focus?
  if (pControl == m_pFocus)
    return;
  ...
  if (pControl != NULL && pControl->GetWindow() == this &&
      pControl->IsVisible() && pControl->IsEnabled()) {
    m_pFocus = pControl;
...
void Window::SetFocusNeeded(Control* pControl) {
  ...
  info.bForward = false;
  m_pFocus =
      m_pRoot->FindControl(__FindControlFromTab, &info,
...
bool Window::IsCaptureControl(const ui::Control* pControl) {
  return m_pEventClick == pControl || m_pEventTouch == pControl ||
         m_pEventPointer == pControl;
}
...
bool Window::SetNextTabControl(bool bForward) {
  ...
  Control* pControl =
      m_pRoot->FindControl(__FindControlFromTab, &info1,
                           UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
  if (pControl == NULL) {
    if (bForward) {
      // Wrap around
...
Control* Window::FindSubControlByName(Control* pParent,
                                      const std::wstring& strName) const {
  if (pParent == NULL)
    pParent = GetRoot();
  ASSERT(pParent);
  return pParent->FindControl(__FindControlFromName, (LPVOID)strName.c_str(),
                              UIFIND_ALL);
}

Control* Window::FindSubControlByClass(Control* pParent,
  ...
  return pParent->FindControl(__FindControlFromClass, (LPVOID)&typeinfo,
                              UIFIND_ALL);
}

std::vector<Control*>* Window::FindSubControlsByClass(
    Control* pParent,
    const type_info& typeinfo) {
  if (pParent == NULL)
    pParent = GetRoot();
...
bool Window::SendNotify(Control* pControl,
                        EventType msgType,
                        WPARAM wParam,
                        LPARAM lParam) {
  EventArgs msg;
...
void Window::Paint() {
  ...
  if (m_bIsArranged && m_pRoot->IsArranged() &&
      (m_pRoot->GetFixedWidth() == DUI_LENGTH_AUTO ||
       m_pRoot->GetFixedHeight() == DUI_LENGTH_AUTO)) {
  ...
  if (m_bIsLayeredWindow) {
    ...
    if (rcWindow.right > cxScreen &&
        rcWindow.left + rcPaint.right == xScreen + cxScreen)
      rcPaint.right = rcClient.right;
    if (rcWindow.bottom > cyScreen &&
        rcWindow.top + rcPaint.bottom == yScreen + cyScreen)
  ...
  if (m_bIsArranged) {
    ...
    if (!::IsRectEmpty(&rcClient)) {
      if (m_pRoot->IsArranged()) {
        ...
        while ((pControl = m_pRoot->FindControl(
                    __FindControlFromUpdate, NULL,
                    UIFIND_VISIBLE | UIFIND_ME_FIRST)) != nullptr) {
  ...
  // alpha修复
  if (m_bIsLayeredWindow) {
    if (m_shadow.IsShadowAttached() && m_renderOffset.x == 0 &&
        m_renderOffset.y == 0) {
  ...
  if (m_bIsLayeredWindow) {
    ...
    CSize szWindow(rcClient.right - rcClient.left,
                   rcClient.bottom - rcClient.top);
    CPoint ptSrc;
    BLENDFUNCTION bf = {AC_SRC_OVER, 0, m_nAlpha, AC_SRC_ALPHA};
    ::UpdateLayeredWindow(m_hWnd, NULL, &pt, &szWindow,
                          m_renderContext->GetDC(), &ptSrc, 0, &bf, ULW_ALPHA);
  } else {
    ::BitBlt(ps.hdc, rcPaint.left, rcPaint.top, rcPaint.GetWidth(),
             rcPaint.GetHeight(), m_renderContext->GetDC(), rcPaint.left,
             rcPaint.top, SRCCOPY);
  }
...
void Window::OnInitLayout() {
  if (m_pRoot && IsWindowsVistaOrGreater()) {
    AnimationPlayer* animationArgs =
        m_pRoot->GetAnimationManager().SetFadeAlpha(true);
    m_nAlpha = 0;
    std::function<void(int)> playCallback =
        nbase::Bind(&Window::SetAlpha, this, std::placeholders::_1);
    animationArgs->SetCallback(playCallback);
    m_pRoot->SetVisible(true);
  }

  // auto fromLeftAnimationPlayer =
  // m_pRoot->GetAnimationManager().GetAnimationPlayer(kAnimationInoutXFromLeft);
  // if (!fromLeftAnimationPlayer && m_shadow.IsShadowAttached()) {
  //	Control* tmpRoot = m_pRoot->GetItemAt(0);
  //	fromLeftAnimationPlayer =
  //tmpRoot->GetAnimationManager().GetAnimationPlayer(kAnimationInoutXFromLeft);
  //}
  // auto fromRightAnimationPlayer =
  // m_pRoot->GetAnimationManager().GetAnimationPlayer(kAnimationInoutXFromRight);
  // if (!fromRightAnimationPlayer && m_shadow.IsShadowAttached()) {
  //	Control* tmpRoot = m_pRoot->GetItemAt(0);
  //	fromRightAnimationPlayer =
  //tmpRoot->GetAnimationManager().GetAnimationPlayer(kAnimationInoutXFromRight);
  //}
  // AnimationPlayer* fadeInoutXPlayer = nullptr;
  // if (fromLeftAnimationPlayer) {
  //	fadeInoutXPlayer = m_pRoot->GetAnimationManager().SetFadeInOutX(true,
  //false);
  //}
  ...
  // if (fadeInoutXPlayer) {
  //	std::function<void(int)> playCallback =
  //nbase::Bind(&Window::SetRenderOffsetX, this, std::placeholders::_1);
...
Control* CALLBACK Window::__FindControlFromNameHash(Control* pThis,
                                                    LPVOID pData) {
...
Control* CALLBACK Window::__FindControlFromCount(Control* /*pThis*/,
                                                 LPVOID pData) {
...
// Control* CALLBACK Window::__FindControlFromShortcut(Control* pThis, LPVOID
// pData)
//{
...
//	if( typeid(*pThis) == typeid(Label) ) return NULL;   // Labels never get
//focus! 	return pFS->bPickNext ? pThis : NULL;
//}

Control* CALLBACK Window::__FindControlFromUpdate(Control* pThis,
                                                  LPVOID pData) {
...
Control* CALLBACK Window::__FindControlFromClass(Control* pThis, LPVOID pData) {
  ...
  std::vector<Control*>* pFoundControls =
      pThis->GetWindow()->GetSubControlsByClass();
...
Control* CALLBACK Window::__FindControlsFromClass(Control* pThis,
                                                  LPVOID pData) {
  type_info* pTypeInfo = static_cast<type_info*>(pData);
  if (typeid(*pThis) == *pTypeInfo)
