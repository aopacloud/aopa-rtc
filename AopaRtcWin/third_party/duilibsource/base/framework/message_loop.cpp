...
void MessageLoop::RemoveAllTask() {
	NAutoLock lock(&incoming_queue_lock_);
	while (!incoming_queue_.empty()) incoming_queue_.pop();
	while (!work_queue_.empty()) work_queue_.pop();
	while (!deferred_non_nestable_work_queue_.empty()) deferred_non_nestable_work_queue_.pop();
